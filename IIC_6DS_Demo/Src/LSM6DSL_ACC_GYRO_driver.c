...
* File Name          : LSM6DSL_ACC_GYRO_driver.c
* Author             : MEMS Application Team
* Version            : v1.6
* Date               : 1 Dec 2016  
* Description        : LSM6DSL source driver file
*
********************************************************************************
 ...
 */
static uint8_t Sensor_IO_Read(void *handle, uint8_t ReadAddr, uint8_t* pBuffer, uint16_t Size )
{

  HAL_StatusTypeDef status = HAL_OK;

  status = HAL_I2C_Mem_Read( handle, LSM6DSL_ACC_GYRO_I2C_ADDRESS_HIGH, ( uint16_t )ReadAddr, I2C_MEMADD_SIZE_8BIT, pBuffer, Size,
                             NUCLEO_I2C_TIMEOUT_MAX );
 ...
 * @retval 1 in case of failure
 */
static uint8_t Sensor_IO_Write(void *handle, uint8_t WriteAddr, uint8_t* pBuffer, uint16_t Size )
{

  HAL_StatusTypeDef status = HAL_OK;

  status = HAL_I2C_Mem_Write( handle, LSM6DSL_ACC_GYRO_I2C_ADDRESS_HIGH, ( uint16_t )WriteAddr, I2C_MEMADD_SIZE_8BIT, pBuffer, Size,
                              NUCLEO_I2C_TIMEOUT_MAX );
...
status_t LSM6DSL_ACC_GYRO_ReadReg(void *handle, u8_t Reg, u8_t* Data, u16_t len) 
{
  ...
  //SPI_Read_MultiData_Raw(Reg|0x80, len, Data);                   //[Example]
  //I2Cx_Read(Data, LSM6DSL_ACC_GYRO_I2C_ADDRESS, Reg, len);       //[Example]
...
status_t LSM6DSL_ACC_GYRO_WriteReg(void *handle, u8_t Reg, u8_t *Data, u16_t len) 
{
  ...
  //SPI_Write_MultiData_Raw(Reg, len, Data);                      //[Example]
  //I2Cx_Write(Data, LSM6DSL_ACC_GYRO_I2C_ADDRESS, Reg, len);     //[Example]
...
status_t  LSM6DSL_ACC_GYRO_W_Stamping_Time_Frame(void *handle, u8_t newValue)
{
  ...
  value &= ~LSM6DSL_ACC_GYRO_TPH_MASK; 
  value |= newValue;
...
status_t  LSM6DSL_ACC_GYRO_W_FIFO_Watermark(void *handle, u16_t newValue)
{
  ...
  value &= ~LSM6DSL_ACC_GYRO_WTM_FIFO_CTRL1_MASK; 
  value |= valueL;
...
status_t LSM6DSL_ACC_GYRO_SH0_WriteByte(void *handle, u8_t SlvAddr, u8_t Reg, u8_t Bufp)
{
  ...
  uint8_t sl0_cfg = 0x10;
  LSM6DSL_ACC_GYRO_WriteReg(handle, LSM6DSL_ACC_GYRO_SLAVE0_CONFIG, &sl0_cfg, 1);
  
  uint8_t sl1_cfg = 0x20;
  LSM6DSL_ACC_GYRO_WriteReg(handle, LSM6DSL_ACC_GYRO_SLAVE1_CONFIG, &sl1_cfg, 1);  

...
#include "clock.h" 
#include "gp_timer.h"
#include "stm32l1xx_hal_bluenrg1_spi.h"
...
{
  //#warning "the pin must be reset as SPI MISO pin"
  GPIO_InitTypeDef GPIO_InitStruct;
  
  /* Configure MOSI pin for bootloader */
  GPIO_InitStruct.Pin = BNRG_SPI_MOSI_BOOT_PIN;
  GPIO_InitStruct.Mode = BNRG_SPI_MOSI_BOOT_UPDATER_MODE;
  GPIO_InitStruct.Pull = BNRG_SPI_MOSI_BOOT_UPDATER_PULL;
  GPIO_InitStruct.Speed = BNRG_SPI_MOSI_BOOT_SPEED;
  HAL_GPIO_Init(BNRG_SPI_MOSI_BOOT_PORT, &GPIO_InitStruct); 
}

void SdkEvalSpiDtmMisoPinState(FunctionalState state)
{
  if(state == ENABLE) {
    HAL_GPIO_WritePin(BNRG_SPI_MOSI_BOOT_PORT, BNRG_SPI_MOSI_BOOT_PIN, GPIO_PIN_SET);
  }
  else {
    HAL_GPIO_WritePin(BNRG_SPI_MOSI_BOOT_PORT, BNRG_SPI_MOSI_BOOT_PIN, GPIO_PIN_RESET);
  }  
}
